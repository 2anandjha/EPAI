...
Original file is located at
    https://colab.research.google.com/drive/159Ubzqqh0HR8ufd-lsaCS_1sZ0p2XIWP
"""

'''single expression that includes lambda, zip and map functions to select create 52 cards in a dec'''

suits = ['spades', 'clubs', 'hearts', 'diamonds']
vals = ['2', '3', '4', '5', '6', '7', '8', '9', '10', 'jack', 'queen', 'king', 'ace']

crds = list(map(lambda x:(x[0], x[1]),zip(vals*4,suits*13)))
print(crds)
...
##2.1
l=[1,2,3,4,5,6]
l2=[2,3,4,5,5,6,7,7,8,5,6,75,7]
'''the list comprehension will take x from l and y from l2
 Expression will use zip to combines and takes input from evalution l on even and l2  on odd
 '''
result = [x+y for x,y in zip(l,l2) if x%2==0 or y%2>0]
print (result)

##2.2
ts='tsai'
v='aeiou'
'''the expression will strip method vowles from the sting '''
#print(t[0])
result = [x.strip(v) for x in ts if v is not ts]
print(result)

##2.3
l=[1,2,3,4,-1,-4,0]
''' relu function returns input ,itslef if it
is > 0 else 0'''
relu =[x for x in l if x>0]
print (relu)

##2.4
'''sigmoid neuron's output is larger than or equal to 0.5,
it outputs 1; if the output is smaller than 0.5, it outputs 0'''
l=[-1,-2,0,.1,1,2,3]
##[.5 for x in l if x <=.5]
result = [.5 if x <=.5 else 1 for x in l]
print(result)
...
a = ord('a')
shift=5
''' refered https://stackoverflow.com/questions/14424500/text-shift-function-in-python'''
result = [''.join(chr((ord(x) - a + shift) % 26 + a) if 'a' <= x <= 'z' else x for x in l)]
print(result)

##3 

def chk_slur(mypara: str) -> bool:
    """
...
l=[1,2,3,4,5,6,7,8]
result = reduce(lambda x, y: x + y, l[2::3])
print(result)

from functools import reduce
mynumbers=[2,4,6,7,89]
'''reduce function on applied over lambda to deduce the sum of all even number'''

## reduce function has taken lambda as the function and the filter as iter
result = reduce(lambda x, y: x + y, filter(lambda x: (x % 2 == 0), mynumbers))
print(result)

mystring='AASHVI'
'''reduce function will use max aprtial function over list'''
